...
class Config:
    ...
    def create_config(self):
        ...
        for directory in secure_dirs:
            os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)), directory), exist_ok=True, mode=0o750)
    ...
    def get_web_secret(self):
        if not os.path.exists(self.config_file):
            self.create_config()
        self.load_config()
        secret = self.config['SECURITY'].get('web_secret', '')
        if not secret or secret == 'CHANGE_ME':
            secret = secrets.token_urlsafe(32)
            self.config['SECURITY']['web_secret'] = secret
            with open(self.config_file, 'w', encoding='utf-8') as configfile:
                self.config.write(configfile)
            os.chmod(self.config_file, 0o600)
        return secret
